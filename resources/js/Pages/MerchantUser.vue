<template>
    ...
    <Table :columns="columns" :url="url" ref="tableData">
        <template #top-left1>
            <q-btn outline label="create" @click="openMerchantUserFormModal('create')" />
        </template>
    ...
    <MerchantUserFormModal v-model="formModal" :action="formAction" :data="rowData"
        @closeModal="closeMerchantUserFormModal" />
</template>
<script setup>
import { ref, watch } from "vue";
import { useQuasar } from "quasar";
import MerchantUserFormModal from "@/Components/MerchantUserFormModal.vue";
defineProps({
    details: Object
})
...
const url = `/table/merchants/${route().params.merchant}/users`;


const openMerchantUserFormModal = (action, data = null) => {
    rowData.value = data;
    formAction.value = action;
    formModal.value = true;
}

const closeMerchantUserFormModal = () => {
...
const onItemClick = (action, data) => {
    if (action == "edit") {
        openMerchantUserFormModal('update', data);
    } else if (action == 'delete') {
        deleteMerchantUser(data);
    }
}

const deleteMerchantUser = (data) => {
    $q.dialog({
        title: 'Are you sure?',
