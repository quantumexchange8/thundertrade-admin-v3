...
watch(modelValue, async (newVal) => {
    if (newVal) {
        ...
        if (props.action == 'update') {
            const { data } = await axios.get(route('merchants.roles.edit', { merchant: route().params.merchant, role: props.data.id }));
            if (data.success) {
...
const onSubmit = () => {
    if (props.action == 'update') {
        axios.put(route('merchants.roles.update', { merchant: route().params.merchant, role: props.data.id }), form.values).then(response => {
        ...
        axios.post(route('merchants.roles.store', { merchant: route().params.merchant }), form.values).then(response => {
            if (response.data.success) {
                $q.notify({ type: 'positive', message: response.data.message })
            } else {
                $q.notify({ type: 'negative', message: response.data.message })
